...
// NEW: Guest upload endpoint for review images (no authentication required)
router.post("/upload-guest", upload.array("images", 5), async (req, res) => {
  try {
    if (!req.files || req.files.length === 0) {
      return res.status(400).json({ message: "No files uploaded" });
    }

    console.log("Starting guest upload for", req.files.length, "files");
    ...
    const uploadWithRetry = async (file, retries = 3) => {
      for (let attempt = 1; attempt <= retries; attempt++) {
        try {
          ...
          // NEW: Use reviews folder for guest uploads
          const result = await cloudinary.uploader.upload(dataUri, {
            folder: "fssmartphones/reviews",
            resource_type: "image",
            use_filename: true,
    ...
    console.error("Guest upload endpoint error:", error);
